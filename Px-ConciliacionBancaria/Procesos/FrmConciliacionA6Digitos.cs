...
{
    public partial class FrmConciliacionA6Digitos : FormaGenBar
    {
        ...
        public FrmConciliacionA6Digitos()
        {
            InitializeComponent();
            Start();
        }
        ...
        private async void detConciliar(object sender, EventArgs e)
        {
            ...
            await _Main.Status($"¿Deseas hacer la Conciliación a 6 Dígitos?", (int)MensajeTipo.Question);
            if (MessageBoxMX.ShowDialog(null, $"¿Deseas hacer la Conciliación a 6 Dígitos?", "Precaución", (int)StatusColorsTypes.Question, true) == System.Windows.Forms.DialogResult.Cancel)
                return;
            ...
            var resultado = await UeMovtosConciliadosAsync(IdBanco, IdCuenta);
            ...
            if (resultado == 1)
            {
                resultado = await UeConciliada(IdBanco, IdCuenta);
        ...
        public async Task<int> UeMovtosConciliadosAsync(string IdBanco, string IdCuenta)
        {
            // Declaración de variables
            string Ls_User = "UsuarioActual"; // Usuario actual del sistema
            DateTime Ld_FechaFin = Idt_Fecha.Date;
            DateTime Ld_FechaIniBco = FProFechaIniMes(Id_VigenciaBco);
            DateTime Ld_FechaIniAux = FProFechaIniMes(Id_VigenciaAux);

            // Verificar si el checkbox "cbx_concilia6" está activado
            if (checkConciliaSimilitud.Checked)
            {
                // Llamada al procedimiento almacenado P_SREF6

                oReq.Tipo = 1;
                oReq.Query = $"EXECUTE P_SREF6 {Ii_Banco}, {Ii_Cuenta}, '{Ld_FechaIniAux:yyyy-MM-dd HH:mm:ss}', " +
                        $"'{Ld_FechaIniBco:yyyy-MM-dd HH:mm:ss}', '{Ld_FechaFin:yyyy-MM-dd HH:mm:ss}', {Il_Conciliacion}";
                ...
                if (oRes.Err != 0)
                {
                    // Mostrar mensaje de error si el procedimiento falla
            ...
            // Retorna 1 si todo se ejecutó correctamente
            return 1;
        }
